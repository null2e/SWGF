...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      let friendly = "로그인 실패";
      if (m.includes("auth/invalid-credential") || m.includes("auth/wrong-password")) friendly = "비밀번호가 올바르지 않습니다.";
      else if (m.includes("auth/user-not-found")) friendly = "존재하지 않는 계정입니다.";
      else if (m.includes("존재하지 않는 아이디")) friendly = "존재하지 않는 아이디입니다.";
      setMsg(`❌ ${friendly}`);
      console.error(err);
    }
  };
  ...
  return (
    <div className="login-page">
      <div className="login-container">
        <h1>Dooop ! 에 오신 것을 <br /> 환영해요 :)</h1>
        <p className="login-subtext">Dooop ! 으로 단어와 일정을 관리하고, 작고 작은 성취의 즐거움을 느껴보세요</p>
