...
export function FriendProvider({ children }) {
      ...
      async (snap) => {
        ...
        const profiles = await Promise.all(
          rows.map(async (r) => {
            ...
            const handle = p.id ?? p.userId ?? r.id;                // 화면에 보일 ID(커스텀 id 우선)
            return { uid: r.id, name, handle };
          })
        );
        setFriends(profiles);
      },
  ...
  const acceptRequest = async (requestIdOrObj) => {
    const requestId = typeof requestIdOrObj === "object" ? requestIdOrObj?.requestId : requestIdOrObj;
    if (!requestId) return;
    try { await respond({ requestId, action: "accept" }); }
    catch (e) { console.error("[acceptRequest]", e?.code, e?.message); alert(e?.message ?? "수락 실패"); }
  };

  const rejectRequest = async (requestIdOrObj) => {
    const requestId = typeof requestIdOrObj === "object" ? requestIdOrObj?.requestId : requestIdOrObj;
    if (!requestId) return;
    try { await respond({ requestId, action: "reject" }); }
    catch (e) { console.error("[rejectRequest]", e?.code, e?.message); alert(e?.message ?? "거절 실패"); }
  };
